...
samples_map <- ggplot(data = world) + geom_sf() + xlab("Latitude") + ylab("Longitude") + ggplot2::coord_sf(xlim = c(-12, 38), ylim = c(30, 65), expand = FALSE) + annotation_scale(location = "bl", width_hint = 0.5) 
```


```{r echo=FALSE, message=FALSE}
map_theme <- theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black",), axis.title.x = element_text(size = 16), axis.text.x = element_text(size = 14, colour = "black"),axis.text.y = element_text(size = 14, colour = "black"),axis.title.y = element_text(size = 16))
```
...
> FIX CITIES COORDINATES!!!! ST ANDREWS IS IN THE SEA!!!! 

```{r echo=FALSE, message=TRUE}

globe_labs <- ggplot(data = world) + geom_sf() + xlab("Latitude") + ylab("Longitude") +
  coord_sf(xlim = c(-90.15, 45.12), ylim = c(-45, 60), expand = TRUE) +
  ...
  annotate(geom = "point", x = cities$Y, y = cities$X, colour = "red", size = 2) 
...
Tables for descriptive statistics at population and line level can be found in the main directory, under the file name **__table_TraitAbbreviation_Level_BatchInfo.csv__** (_i.e. table_Via_Line_wobatch.csv_)
...
```{r, echo=FALSE}
table_Via_Line_wobatch <- write.csv(d_Via %>% group_by(Supervisor.PI, Population, Line) %>% summarise_at(vars(ProportionEggtoAdultSurvival), list(Mean = mean, SD= sd, Median = median, Min = min, Max = max, SE = std_err, CV = coef_var, Mode = estimate_mode)), file = "table_Via_Line_wobatch.csv", row.names = T)
...
capture.output(summary(Via_lm_Schmidt),file = "Via_lm_Schmidt_sum.txt")
capture.output(anova(Via_lm_Schmidt),file = "Via_lm_Schmidt.txt")
capture.output(emmeans(Via_lm_Schmidt, list(pairwise ~ Population), adjust = "tukey"),file = "Via_lm_Schmidt_tk.txt")
```
