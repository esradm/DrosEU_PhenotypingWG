...
```{r d_LS_M_prop, message=FALSE}
d_LS_prop_pop <- d_LS_M %>%
  filter(Censor != 1) %>%
  group_split(Supervisor.PI, Population, Sex) %>%
  lapply(., survProp, cuts = seq(0, 120, 10)) %>%
  bind_rows()
```
...
```{r LS_L_prop}
d_LS_F_Flatt_survival_curves <- d_LS_prop_pop %>%
  filter(Supervisor.PI == "Flatt", Sex == "F") %>%
  mutate(Population = factor(Population, levels = c("YE", "RE", "GI", "MU", "MA", "UM", "KA", "VA", "AK"))) %>%
  ggplot() + 
  geom_line(aes(y = PropSurv, x = AgeAtDeath, colour = Population), size = 0.6) +
  xlim(0, 120) +
  ...
  droseu_theme
d_LS_F_Flatt_survival_curves
```

```{r, include=FALSE}
pdf(file="Lifespan/d_LS_F_Flatt_survival_curves.pdf", width=8, height=5)
d_LS_F_Flatt_survival_curves
invisible(dev.off())
```
