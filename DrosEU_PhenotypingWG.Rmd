...
The consortium has decided to make a collaborative phenotyping effort of European Drosophila melanogaster populations at Groningen meeting, 2017. 

The call was made and participating labs have indicated which traits they are willing to phenotype, with approximate numbers of populations, lines, etc. and under predefined criteria (finish within a year, reps, blocks, etc)
...
```{r}
#knitr::include_graphics("/Volumes/GoogleDrive/My Drive/DrosEU_MSs/20210329/plots/labs_map_edited.pdf")
```
...
droseu_coord <- droseu_coord %>%
  add_column(Collectors = c("Jorge Vieira", "Josefa Gonzales", "Mads Schou", "Amanda Glaser-Schmitt","Andrea Betancourt","Maaria Kankare","Iryna Kozeretska","Banu Onder","Elena Pasyukova"))
```
...
samples_map <- ggplot(data = world) + geom_sf() + xlab("Latitude") + ylab("Longitude") + ggplot2::coord_sf(xlim = c(-12, 38), ylim = c(30, 65), expand = FALSE) + annotation_scale(location = "bl", width_hint = 0.5) + geom_polygon(aes(fill = droseu_coord$Country))
```
...
```{r echo=FALSE, message=FALSE}
#samples_map + panmix_theme + annotate(geom = "text", x = -8.410, y = 41.150, label ="PT",fontface = "bold", color = "black", size =5, angle=15) + annotate(geom = "text", x = 0.620, y = 41.618, label ="ES",fontface = "bold", color = "black", size =5, angle=15) + annotate(geom = "text", x = 10.213, y = 55.945, label ="DK",fontface = "bold", color = "black", size =5, angle=15) + annotate(geom = "text", x = 32.260, y = 40.231, label ="TR",fontface = "bold", color = "black", size =5, angle=15) + annotate(geom = "text", x = 11.610, y = 48.180, label ="DE",fontface = "bold", color = "black", size =5, angle=15) + annotate(geom = "text", x = 15.560, y = 48.375, label ="AT",fontface = "bold", color = "black", size =5, angle=15) + annotate(geom = "text", x = 33.244, y = 57.979, label ="RU",fontface = "bold", color = "black", size =5, angle=15) + annotate(geom = "text", x = 30.206, y = 48.753, label ="UA",fontface = "bold", color = "black", size =5, angle=15) + annotate(geom = "text", x = 23.520, y = 61.100, label ="FI",fontface = "bold", color = "black", size =5, angle=15)
```


```{r echo=FALSE, message=FALSE}
droseu_labs <- read.csv("DrosEU_Labs.csv")
...
Centrally maintained by Élio Sucena  at Instituto Gulbenkian de Ciência (IGC), Lisbon, Portugal
...
Tables for descriptive statistics at population and line level can be found in the main directory, under the file name **__table_TraitAbbreviation_Level_BatchInfo.csv__** (i.e. table_Via_Line_wobatch.csv)
...
```{r, echo=FALSE}
table_Via_Line_wbatch <- write.csv(d_Via %>% group_by(Supervisor.PI, Batch, Population, Line) %>% summarise_at(vars(ProportionEggtoAdultSurvival), list(Mean = mean, SD= sd, Median = median, Min = min, Max = max, SE = std_err, CV = coef_var, Mode = estimate_mode)), file = "table_Via_Line_wbatch.csv", row.names = T)
table_Via_Line_wbatch <- read.csv("table_Via_Line_wbatch.csv")
```

```{r, echo=FALSE}
DT::datatable(table_Via_Line_wbatch, options = list(pageLength = 10))
```

Descriptive statistics for viability at the line level, without batch information : 
```{r, echo=FALSE}
table_Via_Line_wobatch <- write.csv(d_Via %>% group_by(Supervisor.PI, Batch, Population, Line) %>% summarise_at(vars(ProportionEggtoAdultSurvival), list(Mean = mean, SD= sd, Median = median, Min = min, Max = max, SE = std_err, CV = coef_var, Mode = estimate_mode)), file = "table_Via_Line_wobatch.csv", row.names = T)
table_Via_Line_wobatch <- read.csv("table_Via_Line_wobatch.csv")
```

```{r, echo=FALSE}
DT::datatable(table_Via_Line_wobatch, options = list(pageLength = 10))
```

Descriptive statistics for viability at the population level, with batch information : 
```{r, echo=FALSE}
table_Via_Pop_wbatch <- write.csv(d_Via %>% group_by(Supervisor.PI, Batch, Population) %>% summarise_at(vars(ProportionEggtoAdultSurvival), list(Mean = mean, SD= sd, Median = median, Min = min, Max = max, SE = std_err, CV = coef_var, Mode = estimate_mode)), file = "table_Via_Pop_wbatch.csv", row.names = T)
table_Via_Pop_wbatch <- read.csv("table_Via_Pop_wbatch.csv")
```

```{r, echo=FALSE}
DT::datatable(table_Via_Pop_wbatch, options = list(pageLength = 10))
```

Descriptive statistics for viability at the population level, without batch information : 
```{r, echo=FALSE}
table_Via_Pop_wobatch <- write.csv(d_Via %>% group_by(Supervisor.PI, Population) %>% summarise_at(vars(ProportionEggtoAdultSurvival), list(Mean = mean, SD= sd, Median = median, Min = min, Max = max, SE = std_err, CV = coef_var, Mode = estimate_mode)), file = "table_Via_Pop_wobatch.csv", row.names = T)
table_Via_Pop_wobatch <- read.csv("table_Via_Pop_wobatch.csv")
```

```{r, echo=FALSE}
DT::datatable(table_Via_Pop_wobatch, options = list(pageLength = 10))
```


### Plots and Linear Models by Lab

The very same theme has been used all across the document, being **__ggplot theme__**
```{r}
...
Data range is calculated with min and max functions by using `r #min()` and `r #max()` functions 
