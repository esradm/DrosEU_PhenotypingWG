...
_Females_
```{r}
coxme_anova$HSM_F_Parsch_coxme_pop
```
...
_Males_
```{r}
coxme_anova$HSM_M_Parsch_coxme_pop
```
...
_Females_
```{r}
coxme_anova$HSM_F_Vieira_coxme_pop
```
...
The four phenotypes associated with circadian behavior / locomotor activity are shown in __Fig_CET_1__. The diurnal/nocturnal (ND) ratio did not differ significantly between the populations ( _F_ ~8,83~ = 1.57, p = 0.14). However, the populations differed in their circadian period ( _F_~8,95~ = 2.13, _p_ = 0.04) and their daily level of activity ( _F_~8,83~ = 4.59). The differences in acrophase between the populations were marginally significant (Watson-Williams  _F_-test,  _F_~8,95~ = 2.83, _p_ = 0.07). There was no evidence for a correlation between the circadian phenotypes of the different populations, as tested by Spearman rank correlation, except for the ND ratio and the acrophase ( _t_ = -3.7, _df_ = 7, _p_ = 0.007). This is expected, as both traits represent the preferred diel time of activity of the flies.


Eclosion was rhythmic for all populations under LD14:10 and DD conditions at 18° or 29°C ( __Fig_CET_2__ ), with population differences in the eclosion period τ~E~ (MESA), rhythmicity power (JTK_CYCLE and Lomb-Scargle periodogram analysis), eclosion phase Ψ~E~ (MESA) or mean eclosion time (M~E~) ( __Fig_CET_3, Table_CET_1__ ). 


Given the role of the circadian system in timing seasonal events, such as diapause or developmental events like adult eclosion, it is not surprising that various determinants of the circadian clock, such as the circadian free-running period, have been shown to follow geographical clines (reviewed by Hut et al., 2013). For example, earlier studies found clines in eclosion rhythms in northern _Drosophila species_ (Lankinen, 1993, 1986). On the other hand, several studies reported a lack of any geographic cline associated with the period, including the eclosion rhythm of _D. auraria_ (Pittendrigh and Takamura, 1989) and the locomotor activity of the red flour beetle _Tribolium castaneum_ (Abe et al., 2021).


While diapause showed a longitudinal cline in the current study, neither the free-run period nor the acrophase of locomotor activity correlated with latitude or longitude. In parallel, for eclosion, the  population-specific differences in τ, Ψ~E~ and M~E~  as well as the temperature-dependent shift in Ψ~E~ did not significantly correlate with either longitude nor latitude under all conditions (Spearman rank  correlation analysis); overt geographical patterns in τ and Ψ~E~ or rhythmicity or rhythmicity strength were not found.


Various explanations might underlie the lack of geographical clines in the current study. Evidence suggests that the different peripheral oscillators that drive rhythms such as eclosion and locomotor activity are independent (Engelmann and Mack, 1978; Sheeba et al., 2001). Indeed, testing the correlation of eclosion and locomotor rhythm phase of the populations in the current study reveals no significant correlation, although the assays were carried out at different temperatures (Spearman rank correlation, _t_ = 2.2, _df_ = 7, _p_ = 0.07, and _t_ = 0.54, _p_ = 0.6, for eclosion tested at 18°C and 29°C, respectively). The lack of a geographical cline may also be due to a complex climatic pattern that results in bioclimatic conditions that do not change systematically across the populations' habitats along latitude or longitude. As a human commensal (Lachaise and Silvain, 2004), _D. melanogaster_ may encounter such conditions more often than “wild species”. Alternatively, different selective forces may be acting on the different populations in a way that erodes any spatial cline. For example, at high latitudes, some populations may evolve by delaying activity to warmer parts of the day, while others may evolve cold adaptations that allow them to be active during colder parts of the day. Finally, the lack of a clinal pattern in the data may represent limited statistical power. Further research is required to understand the mechanisms that drive the circadian system in wild populations.

$$\\[1cm]$$

__Table_CET_1:__ Eclosion phase ( Ψ~E~, MESA ), mean eclosion time ( M~E~ ), period (τ, MESA) and rhythmicity ( JTK_CYCLE adjusted p with Benjamini-Hochberg correction) for the outcrossed populations ( N = ( 1 ) 2-3 ( 4 ), n=378-3506 eclosed flies, total 39.733 flies)

```{r echo=FALSE}
knitr::include_graphics("CircadianEclosion/FromChris/Table_CET_1.png")
```


```{r echo=FALSE}
knitr::include_graphics("CircadianEclosion/FromChris/Fig_CET_1.png")
```

__Fig_CET_1.__ Locomotor activity phenotypes of wild European D. melanogaster populations. Box plots indicating the median and the interquartile range ( IQR ) of ( __A__ ) ND ratio,( __B__ ) the circadian period, and ( __C__ ) daily time spent in activity. Whiskers signify 1.5 x IQR.  ( __D__ ) The acrophase (phase in DD) is depicted by a circular plot where 0° and 180° correspond to subjective light on (CT0) and light off (CT12), respectively (15°= 1 h). The mean vector and the 95% confidence limits are also depicted. 

$$\\[1cm]$$

```{r echo=FALSE}
knitr::include_graphics("CircadianEclosion/FromChris/Fig_CET_2.png")
```

__Fig_CET_2.__ Averaged eclosion profiles in LD14:10 and DD at 18° and 29°C for the different populations. Arrow bars indicate SE, ( N= ( 1 ) 2-3 ( 4 ), n=378-3506 eclosed flies, total 39.733 flies)

$$\\[1cm]$$

```{r echo=FALSE}
knitr::include_graphics("CircadianEclosion/FromChris/Fig_CET_3.png")
```

__Fig_CET_3.__ Period τ and phase Ψ~E~ in LD14:10 and DD at 18° and 29°C for the different populations. 
...
## Load PCA objects
pca_F13 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA/pca_F13.rds")
pca_F9 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA/pca_F9.rds")
pca_M9 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA/pca_M9.rds")
```
...
```{r echo=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA/plot_F13_1v2.svg")
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA/plot_F13_3v4.svg")
```
...
```{r echo=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA/plot_F9_1v2.svg")
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA/plot_F9_3v4.svg")
```
...
```{r echo=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA/plot_M9_1v2.svg")
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA/plot_M9_3v4.svg")
```
...
fem_mahala <- read.table("MultivariateAnalyses/SI/DFA/MR_female_mahala.txt", header = TRUE)
fem_reallo <- read.table("MultivariateAnalyses/SI/DFA/MR_female_realloc.txt", header = TRUE)
mal_mahala <- read.table("MultivariateAnalyses/SI/DFA/MR_male_mahala.txt", header = TRUE)
mal_reallo <- read.table("MultivariateAnalyses/SI/DFA/MR_male_realloc.txt", header = TRUE)
```
...
knitr::include_graphics("MultivariateAnalyses/SI/DFA/FemaleDis.png")
```
...
knitr::include_graphics("MultivariateAnalyses/SI/DFA/MaleDis.png")
```
...
pca_F13 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/pca_F13.rds")
pca_F9 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/pca_F9.rds")

## Additional PCA objects for testing effect of diet
pca_M8 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/pca_M8.rds")

pcactl_F13 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/pcactl_F13.rds")
pcactl_F9 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/pcactl_F9.rds")
pcactl_M8 <- readRDS("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/pcactl_M8.rds")
```
...
```{r echo=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/plot_diet_F13.svg")
```
...
```{r echo=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/plot_diet_F9.svg")
```
...
```{r echo=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/Phenotype_PCA_ControlDiet/plot_diet_M8.svg")
```
...
ClimVarPCA <- read.csv("MultivariateAnalyses/SI/EnvironmentalAnalyses/ClimVar4PCA_4Rmd.csv")
...
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/Climate_PCA_VecCoord_30d.svg")
```

__PCA for 30 years data__ 

```{r 30y vector and coord plots, echo=FALSE, message=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/Climate_PCA_VecCoord_30y.svg")
```
...
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/Climate_PCA_Scree_30d.svg")
```

__Scree plot for 30 years data__  

```{r 30y scree plot, echo=FALSE, message=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/Climate_PCA_Scree_30y.svg")
```
...
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/Climate_PCA_Loadings_30d.svg")
```

__Loadings for 30 years data__  

```{r 30y loadings, echo=FALSE, message=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/Climate_PCA_Loadings_30y.svg")
```
...
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/Climate_PCA_corrplot_30d.svg")
```

__Correlations for 30 years data__  

```{r 30y correlation plot, echo=FALSE, message=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/Climate_PCA_corrplot_30y.svg")
```
...
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/summary_updated_30y-30d_alt-arrange.svg")
```
```
...
The permutation derived threshold for significance is demarcated by the coloured lines. Permuted data appear as grey points, and real data appear as a coloured point. Only points that have higher -log~10~ P values (i.e. lower P values) and higher R^2^ values pass this statistical test (P and R^2^ are perfectly correlated). Four correlations between phenotype PC2 and climate PC2 passed the permutation procedure (F13 - 30 day, F13 - 30 year, F9 - 30 day, and M9 - 30 day).  

```{r permutations for PC-PC associations, echo=FALSE, message=FALSE}
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/all_permutation_results_v3.svg")
```

#### Phenotype PC2-Climate PC2 correlation 
...
knitr::include_graphics("MultivariateAnalyses/SI/EnvironmentalAnalyses/PhenoPC2_ClimPC2_All_Plots.svg")
```
...
F13_30y_data <- read.csv("MultivariateAnalyses/SI/EnvironmentalAnalyses/F9maxP_30y_data.csv")
F13_30d_data <- read.csv("MultivariateAnalyses/SI/EnvironmentalAnalyses/F9maxP_30d_data.csv")
F9_30y_data <- read.csv("MultivariateAnalyses/SI/EnvironmentalAnalyses/F9_30y_data.csv")
F9_30d_data <- read.csv("MultivariateAnalyses/SI/EnvironmentalAnalyses/F9_30d_data.csv")
M9_30y_data <- read.csv("MultivariateAnalyses/SI/EnvironmentalAnalyses/M9_30y_data.csv")
M9_30d_data <- read.csv("MultivariateAnalyses/SI/EnvironmentalAnalyses/M9_30d_data.csv")

## Load permutation test objects

emp_pr_F13_y <- readRDS("MultivariateAnalyses/SI/EnvironmentalAnalyses/emp_pr_F13_y.rds")
emp_pr_F9_y <- readRDS("MultivariateAnalyses/SI/EnvironmentalAnalyses/emp_pr_F9_y.rds")
emp_pr_M9_y <- readRDS("MultivariateAnalyses/SI/EnvironmentalAnalyses/emp_pr_M9_y.rds")
emp_pr_F13_d <- readRDS("MultivariateAnalyses/SI/EnvironmentalAnalyses/emp_pr_F13_d.rds")
emp_pr_F9_d <- readRDS("MultivariateAnalyses/SI/EnvironmentalAnalyses/emp_pr_F9_d.rds")
emp_pr_M9_d <- readRDS("MultivariateAnalyses/SI/EnvironmentalAnalyses/emp_pr_M9_d.rds")
```
...
- Abe, M.S., Matsumura, K., Yoshii, T., Miyatake, T., 2021. Amplitude of circadian rhythms becomes weaken in the north, but there is no cline in the period of rhythm in a beetle. PLoS ONE 16: e0245115. 
...
- Engelmann, W., Mack, J., 1978. Different oscillators control the circadian rhythm of eclosion and activity in Drosophila. Journal of Comparative Physiology A 127:229–237. 
...
- Hut, R.A., Paolucci, S., Dor, R., Kyriacou, C.P., Daan, S., 2013. Latitudinal clines: An evolutionary view on biological rhythms. Proceedings of the Royal Society B 280:20130433. 
...
- Pittendrigh, C.S., Takamura, T., 1989. Latitudinal Clines in the Properties of a Circadian Pacemaker. Journal of Biological Rhythms 4:105–123.
...
- Sheeba, V., Chandrashekaran, M.K., Joshi, A., Kumar Sharma, V., 2001. A case for multiple oscillators controlling different circadian rhythms in Drosophila melanogaster. Journal of Insect Physiology 47:1217–1225. 
