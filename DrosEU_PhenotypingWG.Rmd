...
samples_map <- ggplot(data = world) + geom_sf() + xlab("Longitude") + ylab("Latitude") + ggplot2::coord_sf(xlim = c(-12, 38), ylim = c(30, 65), expand = FALSE) + annotation_scale(location = "bl", width_hint = 0.5) + ggtitle("Sampling locations")
```
...
globe_labs <- ggplot(data = world) + geom_sf() + xlab("Longitude") + ylab("Latitude") +
  coord_sf(xlim = c(-80.15, 40.12), ylim = c(-45, 60), expand = TRUE) +
  ggtitle("Contributed labs", subtitle = paste0("(", length(unique(droseu_labs$Country)), " countries", ", ", length(unique(droseu_labs$Supervisor.PI)), " research groups", ")")) +
...
```{r, echo=FALSE}
DT::datatable(pval_table, options = list(pageLength = 25)) %>% formatRound(3:7, 4)
```
...
_Females_
```{r fig.show = "hold", out.width = "49%", fig.align = "default", echo = FALSE}
knitr::include_graphics("LinearModelsPop/ChillComa/by_lab_lmer_residuals/CCRT_F_Mensch_lmer_pop_hist_residuals.png")

knitr::include_graphics("LinearModelsPop/ChillComa/by_lab_lmer_residuals/CCRT_F_Mensch_lmer_pop_qq_plot_residuals.png")
```

_Males_
```{r fig.show = "hold", out.width = "49%", fig.align = "default", echo = FALSE}
knitr::include_graphics("LinearModelsPop/ChillComa/by_lab_lmer_residuals/CCRT_M_Mensch_lmer_pop_hist_residuals.png")

knitr::include_graphics("LinearModelsPop/ChillComa/by_lab_lmer_residuals/CCRT_M_Mensch_lmer_pop_qq_plot_residuals.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("MetaAnalyses/ChillComa/CCRT_F_meta_summary_effect.png")
```

_Males_

```{r echo=FALSE}
knitr::include_graphics("MetaAnalyses/ChillComa/CCRT_M_meta_summary_effect.png")
```
...
```{r}
lmers_lat$CCRT_F_Mensch_lmer_lat
```

```{r}
lmers_lat$CCRT_M_Mensch_lmer_lat
```
...
#### Mensch Lab 
```{r}
lmers_lon$CCRT_F_Mensch_lmer_lon
```

```{r}
lmers_lon$CCRT_M_Mensch_lmer_lon
```
...
#### Mensch Lab 
```{r}
lmers_alt$CCRT_F_Mensch_lmer_alt
```

```{r}
lmers_alt$CCRT_M_Mensch_lmer_alt
```
