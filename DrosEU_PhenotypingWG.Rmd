...
```{r p_CCRT_F_meta, fig.width=8, fig.height=5, eval=FALSE, include=FALSE}
  p_CCRT_F_meta_SE <- CCRT_F_meta$summary_effects_random %>% 
    ...
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) +
    annotate("text", x = -Inf, y = Inf, label = sprintf("italic(Q) == %.4g", CCRT_F_meta$Qtest_het$Q), 
             hjust=-0.2, vjust=3.5, parse = T) +
    annotate("text", x = -Inf, y = Inf, label = sprintf("italic(p) == %.4g", CCRT_F_meta$Qtest_het$p), 
             hjust=-0.2, vjust=5, parse = T) +
    theme(legend.position = "none")
...
```{r p_CCRT_M_meta, fig.width=8, fig.height=5, eval=FALSE, include=FALSE}
  p_CCRT_M_meta_SE <- CCRT_M_meta$summary_effects_random %>% 
    ...
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) +
    annotate("text", x = -Inf, y = Inf, label = sprintf("italic(Q) == %.4g", CCRT_M_meta$Qtest_het$Q), 
             hjust=-0.2, vjust=3.5, parse = T) +
    annotate("text", x = -Inf, y = Inf, label = sprintf("italic(p) == %.4g", CCRT_M_meta$Qtest_het$p), 
             hjust=-0.2, vjust=5, parse = T) +
    theme(legend.position = "none")
