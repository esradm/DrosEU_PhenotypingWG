...
dir.create("Wolbachia/resultsLab")
  ...
  for (j in seq(1,length(TR),1)){
    ...
    if (nrow(DATA)>0){
      ...
      if (length(levels(as.factor(DATA$Lab)))>1){
        FIXED=paste0(FIXED," * Lab+(1|Line:Country)+(1|Batch:Lab)")
        TERMS=c("Lab",TERMS)
        ...
        FIXED=paste0(FIXED," + (1|Line:Country)+(1|Batch:Lab)")
