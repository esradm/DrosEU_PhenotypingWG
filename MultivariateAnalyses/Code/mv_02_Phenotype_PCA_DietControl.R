...
masterall <- read.csv("all_models_compound_coefs_forMultivariateAnalysis.csv")
masterctl <- read.csv("all_models_coefsDietControl_forMultivariateAnalysis.csv")
...
### OPTIONAL: Save the various data files ready for PCA
...
# Good congruence of PC1 and PC2 between the full data and controlled diet data

# Summary of F13 diet control PCA
pcactl_F13$eig
pcactl_F13$var$coord
fviz_eig(pcactl_F13)

# Here are the individual coordinates for the qualitative vars
pca_F13_ind<-cbind(datactl_F13[,c(1:3)], pcactl_F13$ind$coord)
pctl_F13_v1<-plot(pcactl_F13,choix="var",axes=c(1,2),cex=1.4) + 
...
# All data and diet controlled data side by side
...
# Summary of F9 diet control PCA
pcactl_F9$eig
pcactl_F9$var$coord
fviz_eig(pcactl_F9)

# Here are the individual coordinates for PC2 output with the qualitative vars
pca_F9_ind<-cbind(datactl_F9[,c(1:3)], pcactl_F9$ind$coord)
pctl_F9_v1<-plot(pcactl_F9,choix="var",axes=c(1,2),cex=1.4) + 
  theme(panel.grid = element_blank())
...
# summary of M8 all data PCA
pca_M8$eig
pca_M8$var$coord
fviz_eig(pca_M8)
fviz_contrib(pca_M8, choice = "var", axes = c(1,3), top = 10)

# Here are the individual coordinates for PC1 output with the qualitative vars
pca_M8_ind<-cbind(datactl_M8[,c(1:3)], pca_M8$ind$coord)
pctl_M8_v1<-plot(pca_M8,choix="var",axes=c(1,2),cex=1.4) + 
  theme(panel.grid = element_blank())


# summary of M8 diet control PCA
pcactl_M8$eig
pcactl_M8$var$coord
fviz_eig(pcactl_M8)
fviz_contrib(pcactl_M8, choice = "var", axes = c(1,3), top = 10)

# Here are the individual coordinates for PC1 output with the qualitative vars
pcactl_M8_ind<-cbind(datactl_M8[,c(1:3)], pcactl_M8$ind$coord)
pctl_M8_v1<-plot(pcactl_M8,choix="var",axes=c(1,2),cex=1.4) + 
  theme(panel.grid = element_blank())
...
# OPTIONAL: save PCA data objects
# saveRDS(pcactl_F13, file = "pcactl_F13.rds")
# saveRDS(pcactl_F9, file = "pcactl_F9.rds")
# saveRDS(pcactl_M8, file = "pcactl_M8.rds")
# saveRDS(pca_M8, file = "pca_M8.rds")


# save.image(file = "PCA_DietControl_results_drosEU.RData")
