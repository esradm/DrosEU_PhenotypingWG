...
#for 30y
climate_data <- as.data.table(climate_data)
setnames(climate_data, c("Dim.1", "Dim.2"), c("PC1_clim", "PC2_clim"))
...
permPlot <- ggplot() +
  ...
  ylab("R^2") + xlab("-log10(p)") + facet_grid(traitPC~envPC) + ggtitle("30 Year")
