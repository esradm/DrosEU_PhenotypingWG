...
# 429 294 428 418 424 338 377 419 423

masterOrd$Country <- as.factor(c(rep("Finland", 429),rep("Spain", 294),rep("Denmark", 428),rep("Austria", 418),
             rep("Germany", 424),rep("Portugal", 338),rep("Ukraine", 377),rep("Russia", 419),rep("Turkey", 423)))

table(master$Model)
# glmer_pop  lmer_pop 
# 158      3392 

masterOrd2 <- subset(masterOrd, Model == "lmer_pop")

# get separate male and female data
masterf <- subset(masterOrd2, Sex == "F")
masterm <- subset(masterOrd2, Sex == "M")
...
p3aa<-p3a+theme(text = element_text(size = 16), 
  ...
  # annotate("text", x = -1.8, y = -1.8, label = "Turkey", cex = 5, colour = "#132B69") +
   annotate("text", x = 0.1, y = -0.4, label = "Spn", cex = 5, colour = "#B82E00") 
# labels can be added later
